...
def obs_lemmatization(x):
    """
    Lemmatizes the observations.

    :param x: The observation token.
...
def radgraph_parse(arguments):
    """
    Parses the RadGraph relations.
    ...
    df_itemized = pd.read_csv(arguments.input_path)

    sid_pid_pairs = df_itemized[['study_id', 'subject_id']].drop_duplicates().values.tolist()

    tuple_lst = []
    print('Preprocessing sentences...')
    for sid, pid in tqdm(sid_pid_pairs):
        idx_sp = (df_itemized['study_id'] == sid) & (df_itemized['subject_id'] == pid)
        df_sp = df_itemized[idx_sp]

        # unique sentence index
        sen_ids = list(df_sp['sentence_ix'].unique())

        for si in sen_ids:
            idx_sen = df_sp['sentence_ix'] == si
            df_sen = df_sp[idx_sen]
                ...
                for _, row_a in df_a.iterrows():
                    ...
                    if sum(idx_o) > 0:
                        ...
                        for _, row_o in df_o.iterrows():
                            ...
                            obs_lst.append(str(row_o['token']).lower())
                            ...
                            if sum(idx_o_m) > 0:
                                ...
                                for _, row_om in df_o_m.iterrows():
                                    ...
                                    if row_om.label == 'OBS-DP':
                                        temp_lst.append(str(row_om['token']).lower())
                                if len(temp_lst) > 0:
                    ...
                    if sum(idx_oo) > 0:
                        ...
                        for _, row_oo in df_oo.iterrows():
                            ...
                            obs_lst.append(str(row_oo['token']).lower())
                    ...
                    # step 6: create tuple of 8 values (sid, pid, sentence_id, sentence, anatomy, obs, label)
                    t_lst = []
                    for i in range(len(obs_lst)):
                        t_lst.append(
                            (sid, pid, si, sen, anatomy_lst[i], obs_lst[i], label_lst[i], obs_modify_lst[i],
            ...
            # if the sentence does not have any ANATOMY token
                ...
                for i in range(len(obs_lst)):
                    t_lst.append(
                        (sid, pid, si, sen, 'unspecified', obs_lst[i], label_lst[i], obs_modify_lst[i],
                         obs_suggestive_lst[i]))
    ...
    df_anatomy_label = pd.DataFrame(df_lst,
                                    columns=['study_id', 'subject_id', 'sen_id', 'sentence', 'anatomy', 'observation',
                                             'label', 'obs_modify', 'obs_suggestive'])
    ...
    df_anatomy_label.to_csv(arguments.output_path, index=False)
